...
<html>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    Select2::create(array(
                        "multiple"=>false,
                        "name"=>"DISTRIBUTORID",
                        "defaultOption"=>array("Select Company"=>0),
    ...
    <div class="report_title">Report Name: AMSF Collection Report </div>
    ...
    <div class="text-right downloadlink">
        ...
        <div class="clear"></div>
    <?php
    $data_queryB = $this->dataStore('FINANCEAMSFCOLLECTIONREPORT');
    $datatype = $this->dataStore('DISTRIBUTORTYPE');
    $dataamountbalance = $this->dataStore('DISTRIBUTORBALANCEAMOUNT');
    $newArrayDept = array();
    $newArray = array();
    ...
    foreach($data_queryB as $row)
    {
        $i++;
            ...
            if( $dj['DISTID'] == $row['DIST_ID']){
            $respectiveNames[] = $dj['TYPENAME'];
            }
            }
            $SN = str_pad(($i + 1), 6, 0, STR_PAD_LEFT);
            $paymentmethod = "";
            if($row['DIST_TRANS_TYPE'] == 1)
            {
                $paymentmethod =  "ONLINE TRANSACTION";
            }
            if($row['DIST_TRANS_TYPE'] == 2)
            {
                $paymentmethod =  "CASH DEPOSIT";
            }
            if($row['DIST_TRANS_TYPE'] == 3)
            {
                $paymentmethod =  "CHEQUE";
            }
           $amountbalace = 0.00;
            foreach($dataamountbalance as $dj1){
                if( $dj1['DISTRIBUTOR_ID'] == $row['DIST_ID']){
                    $amountbalace = $dj1['AMSF_AMOUNT'];
                   }
                   
                }

            $newArray[] =  array(
                                 'NO.' => $i, // Later will update  
                                 'SUBMISSION DATE' => $row['DATE'],
                                 'TRANSACTION NUMBER' => 'TN-' . $row['LEDGER_ID'],
                                 'INVOICE NUMBER' =>'INV' . $SN . '/' . date('Y'),
                                 'DISTRIBUTOR' => $row['DIST_NAME'],
                                 'TYPE' => implode(', ', $respectiveNames),
                                 'PAYMENT METHOD' => $paymentmethod,
                                 'REFFERENCE NUMBER' => $row['PAYMENT_REFERENCE'],
                                 'CODE' => $row['CODE'],
                                 'AMOUNT(RM)' => $row['CREDIT_AMOUNT'],
                                 'PAYMENT BALANCE(RM)' => $amountbalace,
                                 'STATUS' => $row['STATUS'],
                             );
    }
    // $newArray[0] =  array(
