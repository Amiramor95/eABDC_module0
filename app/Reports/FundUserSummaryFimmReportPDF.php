...
class FundUserSummaryFimmReportPDF extends \koolreport\KoolReport
{
    ...
    function setup()
    {
      ...
      $query_params2 = array();
        ...
        $this->src("mysql")
        ->query("Select TP_USER_COMPANY.TP_NAME AS TP_NAME,TP_USER.TP_USERID AS TP_USERID,TP_USER.TP_USER_FNAME AS TP_USER_FNAME,TP_USER.TP_USER_EMAIL AS TP_USER_EMAIL,TP_MANAGE_GROUP.TP_MANAGE_GROUP_NAME AS TP_MANAGE_GROUP_NAME,SETTING_GENERAL.SET_PARAM AS DESIGNATION,TP_USER.TP_USER_TYPE AS TP_USER_TYPE,TP_USER.TP_USER_TELEPHONE AS MOBILE,TP_USER_COMPANY.TP_COMPANY_MOBILE_NO AS OFFICENUMBER
        ...
        LEFT JOIN admin_management.TP_MANAGE_GROUP AS TP_MANAGE_GROUP
        ON TP_MANAGE_GROUP.TP_MANAGE_GROUP_ID = TP_USER.TP_USER_GROUP
        ...
        WHERE TP_USER.TP_USER_TYPE IN (1,2,3)
        ".(($this->params["COMIDS"]!=array(0))?"and TP_USER.TP_COMP_ID in ( :COM_ID)":"")."
        ".(($this->params["GROUPIDS"]!=array(0))?"and TP_USER.TP_USER_GROUP in ( :GROUP_ID)":"")."
         ")
        ->params($query_params)
        ->pipe($this->dataStore("FUNDUSERSUMMARYFIMMREPORT"));
