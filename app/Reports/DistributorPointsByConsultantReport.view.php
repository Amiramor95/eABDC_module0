...
<html>
    ...
    <div class="report-content">
        ...
        <div class="text-center">
            <h1>Consultants by Distribution Point</h1>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="form-group">
                <div class="col-md-6">
                <b>DISTRIBUTOR POINT</b>
                <?php 
                    Select2::create(array(
                        "multiple"=>false,
                        "name"=>"POINTID",
                        "defaultOption"=>array("All"=>0),
                        "dataSource"=>$this->dataStore('DISTRIBUTORPOINTLIST'),
                        "dataBind"=>array(
                            "text"=>"POINTNAME",
                            "value"=>"POINTID",
                        ),
        ...
        <!-- <div class="download_div">
            <form method="post" action="<?php echo config('app.koolreport_server_url');?>/distributorpointbyconsultantpdf">
            <?php
                ...
                ?>
                  ...
                  <input type="hidden" value="<?php echo $this->params["POINTID"]; ?>" name="POINTID" />
                <button class="btn btn-primary">Download Portrait PDF</button>
            </form>
         </div>
         <div class="download_div">
            <form method="post" action="<?php echo config('app.koolreport_server_url');?>/distributorpointbyconsultantlandscapepdf">
            <?php
                foreach ($this->params['DISTRIBUTORIDS'] as  $value) {
                ?>
                  <input type="hidden" name="DISTRIBUTORIDS[]" value="<?php echo $value; ?>">
         ...
         <div class="download_div">
            <form method="post" action="<?php echo config('app.koolreport_server_url');?>/distributorpointbyconsultantexcel">
            <?php
                foreach ($this->params['DISTRIBUTORIDS'] as  $value) {
                ?>
                  <input type="hidden" name="DISTRIBUTORIDS[]" value="<?php echo $value; ?>">
        ...
        <div class="clear"></div>
    <?php
    $datacon = $this->dataStore('DISTRIBUTORPOINTBYCONSULTANTREPORT');
    ...
    foreach($datacon as $row)
    {
      ...
      $utsdate = "";
      $prsdate = "";
      if($row['CONSULTANT_TYPE_ID'] == 1)
      {
          $uts = 'Y';
          $utsdate =  date("d-M-Y", strtotime($row['CREATED_AT']));
      }
      if($row['CONSULTANT_TYPE_ID'] == 2)
      {
          $prs = 'Y';
          $prsdate = date("d-M-Y", strtotime($row['CREATED_AT']));
      }
            ...
            $newArrayCon[] =  array( 
                                  ...
                                  'STATE' => $row['STATENAME'],
                                  'CONSULTANT NAME' => $row['CONSULTANT_NAME'],
                                  'FIMM NO' => $row['FIMMNO'],
                                  'UTS' => $uts,
                                  'UTS REGISTRATION DATE' => $utsdate,
                                  'PRS' => $prs,
                                  'PRS REGISTRATION DATE' => $prsdate,
