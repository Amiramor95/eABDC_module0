...
<html>
    ...
    <div class="report-content">
        ...
        <div class="text-center">
            <h1>Consultants Registration Report</h1>
        </div>
              ...
              //"format"=>"YYYY-M-D",
                ...
                <?php 
                    Select2::create(array(
                        ...
                        "name"=>"ATYPEID",
                        "defaultOption"=>array("All"=>0),
                        "dataSource"=>$this->dataStore('CONSULTANTAPPLICATION'),
                        "dataBind"=>array(
                            "text"=>"APPLICATION_TYPE_NAME",
                            "value"=>"ATYPEID",
                        ),
                        "attributes"=>array(
                            "class"=>"form-control"
                ...
                <div class="form-group">
                <div class="col-md-6">
                <b>REGISTRATION SUB TYPE</b>
                <?php 
                    Select2::create(array(
                        "multiple"=>false,
                        "name"=>"SUBID",
                        "defaultOption"=>array("All"=>0),
                        "dataSource"=>$this->dataStore('CONSULTANTREGSUBTYPE'),
                        "dataBind"=>array(
                            "text"=>"SUBNAME",
                            "value"=>"SUBID",
                        ),
        ...
        <!-- <div class="download_div">
            <form method="post" action="<?php echo config('app.koolreport_server_url');?>/consultantregistrationpdf">
                <?php
                ...
                <input type="hidden" value="<?php echo $this->params["ATYPEID"]; ?>" name="ATYPEID" />
                <input type="hidden" value="<?php echo $this->params["SUBID"]; ?>" name="SUBID" />
         ...
         <div class="download_div">
            <form method="post" action="<?php echo config('app.koolreport_server_url');?>/consultantregistrationlandscapepdf">
            <?php
                foreach ($this->params['DISTRIBUTORIDS'] as  $value) {
                ?>
                  <input type="hidden" name="DISTRIBUTORIDS[]" value="<?php echo $value; ?>">
         ...
         <div class="download_div">
            <form method="post" action="<?php echo config('app.koolreport_server_url');?>/consultantregistrationexcel">
            <?php
                foreach ($this->params['DISTRIBUTORIDS'] as  $value) {
                ?>
                  <input type="hidden" name="DISTRIBUTORIDS[]" value="<?php echo $value; ?>">
        ...
        <div class="clear"></div>
    <?php
    $datacon = $this->dataStore('CONSULTANTREGISTRATIONREPORT');
    ...
    foreach($datacon as $row)
    {
        ...
        // foreach($newArrayApp as $dj1){
        //     if( $dj1['CONSULTANTID'] == $row['CID'] && $dj1['TYPEID'] == $row['CTYPEID']){
        //         $respectiveUpdateBy[] = $dj1['LATESTUPDATEBY'];
        //         $respectiveUpdate[] = $dj1['LATESTUPDATE'];
        //     }
        // }
        ...
        if( $row['CONSULTANT_ID'] == $row['CID'] && $row['CONSULTANT_TYPE_ID'] == $row['CTYPEID']){
            ...
            $newArrayCon[] =  array( 
                                 ...
                                 'FIMM NO' => $row['FIMMNO'],
                                  ...
                                  'REG TYPE' => $row['APPLICATIONNAME'],
                                  'SUBTYPE' => $row['SUBTYPE'],
                                  'CMSRL No.' => $row['CMSRL_NO'],
                                  'APPROVAl DATE' => implode(', ', $respectiveUpdate),
                                  'APPROVED By' => implode(', ', $respectiveUpdateBy),
                             );
    }
