...
<html>
        ...
        <div class="row form_row">
            <div class="col-md-6">
                ...
                CheckBoxList::create(array(
                    "multiple"=>true,
                    "name"=>"GROUPIDS",
                    "dataSource"=>$this->dataStore("GROUPLIST"),
                    "dataBind"=>array(
                                "text"=>  "GD",
                                "value"=>"GROUPID",
                            ),
    ...
    <div class="text-right">
        <!-- <div class="download_div">
            <form method="post" action="<?php echo config('app.koolreport_server_url');?>/adminusermatrixpdf">
                ...
                ?>
                <?php
                foreach ($this->params['MODULEIDS'] as  $value2) {
                ?>
                 <input type="hidden" name="MODULEIDS[]" value="<?php echo $value2; ?>">
                <?php
                }
                ?>
    ...
    foreach($dataProcessFlow as $rowProcess)
    {
        foreach($dataDepartMent as $key => $value){
            ${"authLevel$key"} =  '<div class="com_header">'.$value['AUTHORIZATION_LEVEL_NAME'].'</div>'.' <br><div class="dept_name">'.$value['DPMT_SNAME'].'</div>';
            ...
            if (in_array($rowProcess['MANAGE_SCREEN_ID'],  $decoded_json )) {
               // echo "Got =" . $rowProcess['PROCESS_FLOW_ID'];
                ${"authLevelData$key"} = '<div style="text-align:center;"><i  class="green fa fa-check"></i></div>';//"1 ------ > " . $rowProcess['PROCESS_FLOW_ID'] ;
            }else{
                ${"authLevelData$key"} = '<div style="text-align:center;"><i class="red fa fa-times" aria-hidden="true"></i></div>'; //' 0 --->' . $rowProcess['PROCESS_FLOW_ID'];
            }
        }
        $newProcessArray[] =  array(
